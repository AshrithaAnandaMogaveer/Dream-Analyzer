...
const RATE_LIMIT_MAX_CONCURRENT = Number(process.env.OAI_MAX_CONCURRENT || 4);
...
const GLOBAL_TIMEOUT_MS = Number(process.env.AI_ANALYSIS_TIMEOUT_MS || 4000);
...
async function callOpenAIWithFallback(callFn, options = {}) {
  try {
    ...
    if (isTimeout && FALLBACK_MODEL) {
      return safeOpenAI(() => callFn({ model: FALLBACK_MODEL }), { timeoutMs: Math.max(3000, GLOBAL_TIMEOUT_MS - 500) });
    }
    throw err;
  }
}
